...
import { cyberlabRoutes } from './app/api/routes/cyberlab.js';
import { teacherCyberlabRoutes } from './app/api/routes/teacherCyberlab.js';
import { learningPathsRoutes } from './app/api/routes/learningPaths.js';
...
const _pkgFile = path.join(path.dirname(fileURLToPath(import.meta.url)), '../../package.json');
let _pkgVersion = 'unknown';
try { _pkgVersion = JSON.parse(readFileSync(_pkgFile, 'utf-8')).version ?? 'unknown'; } catch { /* ignore */ }
export const APP_VERSION = _pkgVersion;

const server = Fastify({
  logger: isDevelopment
    ? true  // Fastify dev logger: human-readable, no extra deps needed
    : {
        level: 'info',
        redact: {
...
// Attach a unique requestId to every request and expose it in the response header.
server.addHook('onRequest', async (request, reply) => {
  const id = (request.id as string) || randomUUID();
...
await server.register(cyberlabRoutes, { prefix: '/api/cyberlab' });
await server.register(teacherCyberlabRoutes, { prefix: '/api/teacher/cyberlab' });
...
import { seedTracks } from './app/services/musicService.js';
...
server.get('/health', async (_request, reply) => {
  ...
  return dbOk ? payload : reply.status(503).send({ ...payload, status: 'degraded' });
});
