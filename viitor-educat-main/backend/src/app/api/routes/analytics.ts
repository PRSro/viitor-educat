...
export async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  /**
   * GET /analytics/student/heatmap
   * Get student activity heatmap data
   */
  fastify.get('/student/heatmap', {
    ...
    try {
      ...
      const activity = await prisma.$queryRaw<{ date: Date; count: bigint }[]>`
        SELECT DATE("completedAt") as date, COUNT(*) as count
        FROM "LessonCompletion"
        WHERE "userId" = ${user.id} AND "completedAt" >= ${startDate}
        GROUP BY DATE("completedAt")
        ORDER BY date ASC
      `;
      ...
      for (let i = numDays - 1; i >= 0; i--) {
        ...
        d.setDate(d.getDate() - i);
        const iso = d.toISOString().split('T')[0];
        heatmapMap.set(iso, 0);
      }

      for (const row of activity) {
        const isoDate = new Date(row.date).toISOString().split('T')[0];
        if (heatmapMap.has(isoDate)) heatmapMap.set(isoDate, Number(row.count) * 15); // mock 15 minutes per lesson
      }

      const heatmap = Array.from(heatmapMap.entries()).map(([date, minutes]) => ({ date, minutes }));

      return { heatmap, streak: userPoints?.streak || 0 };
    } catch (error) {
      fastify.log.error(error, 'Error fetching student heatmap');
      throw error;
    }
  });
}
