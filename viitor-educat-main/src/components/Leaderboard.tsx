...
import { Trophy, Star, Medal, User as UserIcon, Terminal } from 'lucide-react';
...
export function Leaderboard() {
  const [activeTab, setActiveTab] = useState<'total' | 'cyberlab'>('total');

  const { data: leaderboard, isLoading, error } = useQuery({
    queryKey: ['leaderboard', activeTab],
    queryFn: () => pointsService.getLeaderboard(activeTab as any),
  });
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-gray-300">
              Top performing students on the platform
            </CardDescription>
          </div>
          {myPoints && (
        ...
        <div className="flex items-center gap-4 mt-6">
          <button 
            ...
            className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors border ${activeTab === 'total' ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-300' : 'bg-transparent border-transparent text-gray-400 hover:text-gray-200 hover:bg-white/5'}`}
          >
            Global XP
          </button>
          <button 
            onClick={() => setActiveTab('cyberlab')}
            className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors flex items-center gap-2 border ${activeTab === 'cyberlab' ? 'bg-[#00ff88]/10 border-[#00ff88]/50 text-[#00ff88]' : 'bg-transparent border-transparent text-gray-400 hover:text-gray-200 hover:bg-white/5'}`}
          >
            <Terminal className="w-4 h-4" /> Hall of Fame
          </button>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
              ...
              >
                ...
                <div className="flex items-center gap-2 font-bold text-emerald-400">
                  <div className="flex flex-col items-end">
                    ...
                    {activeTab === 'cyberlab' && (
                      <span className="text-[10px] text-[#00ff88]/60 font-mono italic">Solves: {entry.solveCount}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {(!leaderboard || leaderboard.length === 0) && (
            <div className="text-center py-10 space-y-2">
              ...
              <p className="text-gray-500 text-sm">Complete a lesson or quiz to earn points and appear here.</p>
            </div>
          )}
        </div>
      </CardContent>
