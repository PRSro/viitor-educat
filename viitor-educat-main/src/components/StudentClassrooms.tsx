...
export function StudentClassrooms() {
  ...
  const joinMutation = useMutation({
    ...
    onError: (error: any) => {
      setJoinError(error.response?.data?.error || 'An error occurred while enrolling');
    }
  });
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isJoinOpen} onOpenChange={setIsJoinOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Join using your received code</DialogTitle>
            </DialogHeader>
            <div className="py-4 space-y-4">
              <Label htmlFor="code" className="text-gray-300">Class Code (6 characters)</Label>
              <Input
                id="code"
                value={joinCode}
                onChange={(e) => {
            ...
            <DialogFooter>
              <Button disabled={joinMutation.isPending || joinCode.length !== 6} onClick={() => joinMutation.mutate(joinCode)}>
                {joinMutation.isPending ? 'Verifying...' : 'Join'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <CardContent className="pb-2">
               <div className="p-3 mt-2 rounded-lg border border-cyan-500/20 bg-cyan-500/10 flex items-center justify-between">
                <div className="flex gap-2 items-center text-cyan-300">
                   ...
                   <span className="text-sm font-medium">{classroom._count?.classroomLessons || 0} lessons received</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {!classrooms?.length && (
          <div className="col-span-full py-12 text-center border-2 border-dashed border-white/10 rounded-xl">
             ...
             <p className="text-gray-400">You are not enrolled in any class yet.</p>
          </div>
        )}
      </div>
    </div>
