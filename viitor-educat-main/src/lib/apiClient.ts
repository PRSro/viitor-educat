...
export async function apiRequest<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Show a user-friendly toast for 5xx errors with the request ID so they can report it.
      if (response.status >= 500) {
        const requestId = response.headers.get('X-Request-Id');
        const hint = requestId ? ` (ref: ${requestId})` : '';
        toast.error(`Something went wrong on our end${hint}. Please try again shortly.`);
      }
