...
export async function getArticles(filters?: ArticleFilters): Promise<ArticlesResponse> {
  ...
  return api.get(`${API_PATHS.ARTICLES}${queryString ? `?${queryString}` : ''}`);
}
...
export async function getLatestArticles(): Promise<ArticleListItem[]> {
  try {
    const data = await api.get<{ articles: ArticleListItem[] }>(API_PATHS.ARTICLES_LATEST);
...
export async function getArticlesByTeacher(teacherId: string, page = 1, limit = 10): Promise<ArticlesResponse> {
  ...
  return api.get(`${API_PATHS.ARTICLES_TEACHER(teacherId)}?${params}`);
}
...
export async function getArticle(id: string): Promise<Article> {
  const data = await api.get<{ article: Article }>(API_PATHS.ARTICLE(id));
  return data.article;
}
...
export async function getCategories(): Promise<ArticleCategory[]> {
  const data = await api.get<{ categories: ArticleCategory[] }>(API_PATHS.ARTICLES_CATEGORIES);
...
export async function createArticle(articleData: CreateArticleData): Promise<Article> {
  const data = await api.post<{ article: Article }>(API_PATHS.ARTICLES, articleData);
  return data.article;
}
...
export async function importArticle(url: string): Promise<Article> {
  const data = await api.post<{ article: Article }>(API_PATHS.ARTICLE_IMPORT, { url });
  return data.article;
}
...
export async function updateArticle(id: string, articleData: Partial<CreateArticleData & { published: boolean }>): Promise<Article> {
  const data = await api.put<{ article: Article }>(API_PATHS.ARTICLE(id), articleData);
  return data.article;
}
...
export async function deleteArticle(id: string): Promise<void> {
  await api.delete(API_PATHS.ARTICLE(id));
}
