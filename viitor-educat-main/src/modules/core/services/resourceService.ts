...
export async function getResources(filters?: ResourceFilters): Promise<ResourcesResponse> {
  ...
  return api.get(`${API_PATHS.RESOURCES}${queryString ? `?${queryString}` : ''}`);
}
...
export async function getResourcesByLesson(lessonId: string): Promise<ResourceListItem[]> {
  const data = await api.get<{ resources: ResourceListItem[] }>(`${API_PATHS.RESOURCES_LESSON(lessonId)}`);
...
export async function getResourcesByTeacher(teacherId: string): Promise<ResourceListItem[]> {
  const data = await api.get<{ resources: ResourceListItem[] }>(`${API_PATHS.RESOURCES_TEACHER(teacherId)}`);
  return data.resources;
}
...
export async function getResourceById(id: string): Promise<ExternalResource> {
  const data = await api.get<{ resource: ExternalResource }>(API_PATHS.RESOURCE(id));
...
export async function getResourceTypes(): Promise<ResourceType[]> {
  const data = await api.get<{ types: ResourceType[] }>(API_PATHS.RESOURCES_TYPES);
  return data.types;
}
...
export async function createResource(resourceData: CreateResourceData): Promise<ExternalResource> {
  const data = await api.post<{ resource: ExternalResource }>(API_PATHS.RESOURCES, resourceData);
  return data.resource;
}
...
export async function updateResource(id: string, resourceData: Partial<CreateResourceData>): Promise<ExternalResource> {
  const data = await api.put<{ resource: ExternalResource }>(API_PATHS.RESOURCE(id), resourceData);
  return data.resource;
}
...
export async function deleteResource(id: string): Promise<void> {
  await api.delete(API_PATHS.RESOURCE(id));
}
