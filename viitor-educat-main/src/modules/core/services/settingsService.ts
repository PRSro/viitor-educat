...
export async function getSettings(): Promise<UserSettings> {
  const data = await api.get<{ settings: UserSettings }>(API_PATHS.SETTINGS);
...
export async function updateSettings(settings: UpdateSettingsData): Promise<UserSettings> {
  const data = await api.put<{ settings: UserSettings }>(API_PATHS.SETTINGS, settings);
  return data.settings;
}
...
export async function resetSettings(): Promise<UserSettings> {
  const data = await api.post<{ settings: UserSettings }>(API_PATHS.SETTINGS_RESET, {});
  return data.settings;
}
...
export async function getAvailableCategories(): Promise<string[]> {
  const data = await api.get<{ categories: string[] }>(API_PATHS.SETTINGS_CATEGORIES);
