...
export const AdmissionsSection = () => {
  ...
  useEffect(() => {
    ...
    registerElement(bubble1Ref.current, 0.4, 20);
    registerElement(bubble2Ref.current, 0.6, 25);
    registerElement(bubble3Ref.current, 0.5, 18);
    registerElement(bubble4Ref.current, 0.35, 15);
    registerElement(glow1Ref.current, 0.2, 10);
    registerElement(glow2Ref.current, 0.25, 12);
  ...
  return (
    <section id="admitere" className="py-24 lg:py-32 relative overflow-hidden">
      ...
      <div 
        ref={bubble1Ref}
        className="absolute top-[15%] left-[10%] w-28 h-28 rounded-full bg-white/10 animate-float-slow will-change-transform"
      >
        <div className="absolute top-[12%] left-[18%] w-[35%] h-[20%] bg-white/40 rounded-full blur-[2px] rotate-[-20deg]" />
      </div>
      <div 
        ref={bubble2Ref}
        className="absolute top-[40%] right-[8%] w-20 h-20 rounded-full bg-white/8 animate-float will-change-transform" 
      ...
      <div 
        ref={bubble3Ref}
        className="absolute bottom-[25%] left-[5%] w-16 h-16 rounded-full bg-white/8 animate-float-delayed will-change-transform" 
      />
      <div 
        ref={bubble4Ref}
      ...
      <div 
        ref={glow1Ref}
        className="absolute top-1/4 left-0 w-[500px] h-[500px] bg-white/10 rounded-full blur-[150px] will-change-transform" 
      />
      <div 
        ref={glow2Ref}
        className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-accent/15 rounded-full blur-[120px] will-change-transform" 
      />

      <div className="container mx-auto px-4 lg:px-8 relative">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/15 backdrop-blur-md border border-white/30 text-white text-sm font-medium mb-6 relative overflow-hidden">
            <Users className="w-4 h-4" />
            Admitere 2025
            <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/20 to-transparent" />
          </span>
          <h2 className="text-display-sm lg:text-display-md text-white mb-6 text-glow-white">
          ...
          {steps.map((step, index) => (
            <div key={step.title} className="relative group">
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-10 left-full w-full h-0.5 bg-gradient-to-r from-white/30 to-transparent z-0" />
              )}
              
              <div className="relative bg-white/10 backdrop-blur-md border border-white/25 rounded-2xl p-6 hover:bg-white/20 hover:border-white/40 transition-all duration-300 group-hover:shadow-glow overflow-hidden">
                {/* Shine */}
                ...
                <div className="relative">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-white/25 to-white/10 flex items-center justify-center group-hover:from-accent group-hover:to-teal-500 transition-all duration-300 relative overflow-hidden">
                      <step.icon className="w-6 h-6 text-white" />
                      <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/30 to-transparent" />
                    </div>
                    <span className="text-3xl font-bold text-white/25 group-hover:text-white/40 transition-colors duration-300">
                      {step.step}
                    </span>
                  </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="space-y-6">
            <div className="bg-white/10 backdrop-blur-md border border-white/25 rounded-2xl p-6 relative overflow-hidden">
              ...
              <div className="relative">
                <h4 className="font-semibold text-lg text-white mb-3">📅 Date Importante</h4>
                <ul className="space-y-2 text-white/75">
                  <li>• <strong className="text-white">1 Martie:</strong> Deschidere înscrieri online</li>
                  <li>• <strong className="text-white">30 Aprilie:</strong> Termen limită înscriere</li>
                  <li>• <strong className="text-white">15 Iunie:</strong> Evaluare Națională</li>
                  <li>• <strong className="text-white">1 Iulie:</strong> Publicare rezultate</li>
                </ul>
              </div>
            </div>
            <div className="bg-gradient-to-br from-accent/30 to-teal-500/30 backdrop-blur-md border border-white/25 rounded-2xl p-6 relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-1/2 bg-gradient-to-b from-white/15 to-transparent pointer-events-none rounded-t-2xl" />
              <div className="absolute top-4 right-4 w-12 h-12 rounded-full bg-white/10">
                <div className="absolute top-[15%] left-[20%] w-[30%] h-[18%] bg-white/30 rounded-full blur-[1px]" />
              </div>
              <div className="relative">
                <h4 className="font-semibold text-lg text-white mb-3">💡 Sfat</h4>
