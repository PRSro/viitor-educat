...
import { GlassCard } from '@/components/ParallaxLayout';
...
export function CourseEditor({
  ...
  const [status, setStatus] = useState<'DRAFT' | 'PRIVATE' | 'PUBLIC' | 'PUBLISHED'>(course?.status as any || 'DRAFT');
    ...
    <div className="container mx-auto py-8 px-4 max-w-6xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <Badge variant={status === 'PUBLISHED' || status === 'PUBLIC' ? 'default' : status === 'PRIVATE' ? 'secondary' : 'outline'} className="text-sm px-3 py-1">
            {(status === 'PUBLISHED' || status === 'PUBLIC') && <Globe className="w-3 h-3 mr-1" />}
            {status === 'PRIVATE' && <Lock className="w-3 h-3 mr-1" />}
            {status === 'DRAFT' && <FileStack className="w-3 h-3 mr-1" />}
            {status}
          </Badge>
          <Select value={status} onValueChange={(v: 'DRAFT' | 'PRIVATE' | 'PUBLIC' | 'PUBLISHED') => setStatus(v)}>
                  ...
                  {lessonList.map((lesson, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant={lesson.status?.toUpperCase() === 'PUBLIC' ? 'default' : 'secondary'}>
                          {lesson.status || 'DRAFT'}
                        </Badge>
                        <Button
                          variant="ghost"
